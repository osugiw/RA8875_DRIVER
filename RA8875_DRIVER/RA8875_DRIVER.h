...
#ifndef _RA8875_SPI_H
#define _RA8875_SPI_H
...
#define RA8875_BGTR0_ADDR               0x67 // Background Color Register 0 - bit -> 7-5: NA, 4-0: Background Color for Transparent Red
#define RA8875_BGTR1_ADDR               0x68 // Background Color Register 1 - bit -> 7-6: NA, 5-0: Background Color for Transparent Green
#define RA8875_BGTR2_ADDR               0x69 // Background Color Register 2 - bit -> 7-5: NA, 4-0: Background Color for Transparent Blue
...
#define RA8875_BGTR2_ADDR               0x67  // Background color register for transparent 2 - Bit 7-5: NA, 4-0: Background color register for transparent Blue[4:0]
...
class TFT_RA8875{
  private:
    /*  Pins Assignments */
    uint8_t RA8875_WAIT, RA8875_SCLK, RA8875_MISO, RA8875_MOSI, RA8875_CS, RA8875_INT, RA8875_RST; 
    ...
    uint8_t _rotation;
    uint8_t _voffset;
    int16_t _window_color;
    int16_t _bg_text_color;
    uint16_t _previous_text;
    ...
    /*  Drawing Helper  */
    void rectHelper(uint16_t x, uint16_t y, uint16_t w, uint16_t h, int16_t color, bool filled);
    void circleHelper(uint16_t x, uint16_t y, uint16_t r, int16_t color, bool filled);
    void ellipseHelper(uint16_t x, uint16_t y, uint16_t long_axis, uint16_t short_axis, int16_t color, bool filled);
    void curveHelper(uint16_t x, uint16_t y, uint16_t long_axis, uint16_t short_axis, uint8_t curvePart, int16_t color, bool filled);
    void triangleHelper(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, int16_t color, bool filled);
    void roundedRectHelper(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, int16_t color, int16_t filled);

    /*  Window Variable*/
    uint8_t HSSW, HESW, VSSW, VESW, HOFS, VOFS;
  ...
  public:
    TFT_RA8875( uint8_t _wait, uint8_t _sclk, uint8_t _miso, uint8_t _mosi, uint8_t _cs, uint8_t _int, uint8_t _rst);

    /*  Graphics Mode */
    void enableGraphicsMode(void);
    void graphicSetCoordinate(uint16_t x, uint16_t y);
    void drawPixel(uint16_t x, uint16_t y, int16_t color);
    void drawPixels(int16_t x, int16_t y, uint16_t *p, uint32_t count);
    void uploadCustomCharacter(const uint8_t dataBuffer[], uint8_t cgramAddr);
    void printCustomCharacter(int16_t x, int16_t y, uint8_t cgramAddr, uint8_t wide, int16_t foreColor, int16_t bgColor, bool transparent);
    void drawGraphicRAM(uint16_t x, uint16_t y, int16_t color); 
    void bmpDraw(const char *filename, int x, int y);
    void bmpDrawFast(const char *filename, int x, int y);
    void pushPixels(uint32_t num, uint16_t* p);

    /*  Text Mode*/
    void enableTextMode(void);
    void setTextCoordinate(uint16_t x, uint16_t y);
    void setTextColor(int16_t foreColor, int16_t bgColor, bool transparent);
    void textEnlarge(uint8_t scale);
    void clearText(uint16_t x, uint16_t y, uint16_t len, int16_t bgColor);
    void selectInternalFont(uint8_t fnt);
    void cursorBlink(uint8_t frame_rate);
    void verticalText(uint8_t vdir, uint8_t hdir);
    void horizontalText(uint8_t vdir, uint8_t hdir);
    void writeText(uint16_t x, uint16_t y, const char* buffer, int16_t foreColor, int16_t bgColor, uint8_t scale, bool transparent);

    /* System Configuration */
    void PLL_init(void);
    void window_init(void);

    /* System & Power Function */
    boolean init();
    void softwareReset(void);
    void displayOn(void);
    void displayOff(void);
    boolean waitPoll(uint8_t register_address, uint8_t flag);
    uint16_t get_width(void);
    uint16_t get_height(void);
    int8_t get_Rotation(void);
    int16_t get_window_color(void);
    void set_rotation(int8_t rotation);   
    
    /*  Touch Enabled Function  */
    void enableTouch(void);
    boolean isTouched(void);
    boolean readTouch(uint16_t* x, uint16_t* y);
    void brushDraw(int16_t color);
    void pencilDraw(int16_t color, uint8_t thickness);

    /*  IO & Key Control Function */
    void enableGPIOX(void);
    void disableGPIOX(void);

    /* Window Function  */
    void setBackgroundWindow(uint16_t color);
    void setScrollWindowRange(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t scroll_mode);
    void scrollWindowX(uint16_t x_offset);
    void scrollWindowY(uint16_t y_offset);
    // void selectLayer(uint8_t l);
    void setActiveWindow(void);

    /*  Coloring Function */
    void foregroundColor(uint16_t color);
    void backgroundColor(uint16_t color);

    /*  Drawing Function  */
    void drawFilledRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, int16_t color);
    void drawEmptyRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, int16_t color);
    void drawFilledCircle(uint16_t x, uint16_t y, uint16_t r, int16_t color);
    void drawEmptyCircle(uint16_t x, uint16_t y, uint16_t r, int16_t color);
    void drawFilledEllipse(uint16_t x, uint16_t y, uint16_t long_axis, uint16_t short_axis, int16_t color);
    void drawEmptyEllipse(uint16_t x, uint16_t y, uint16_t long_axis, uint16_t short_axis, int16_t color);
    void drawFilledCurve(uint16_t x, uint16_t y, uint16_t long_axis, uint16_t short_axis, uint8_t curvePart, int16_t color);
    void drawEmptyCurve(uint16_t x, uint16_t y, uint16_t long_axis, uint16_t short_axis, uint8_t curvePart, int16_t color);
    void drawLine(uint16_t x, uint16_t y, uint16_t x1, uint16_t y1, int16_t color);
    void drawFilledTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, int16_t color);
    void drawEmptyTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, int16_t color);
    void drawFilledRoundedRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, int16_t color);
    void drawEmptyRoundedRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t r, int16_t color);

    /*  BTE Function */
    void BTE_sourceHelper(int16_t source_hor, int16_t source_ver);
    void BTE_destinationHelper(int16_t dest_hor, int16_t dest_ver);
    void BTE_sizeHelper(uint16_t BTE_width, uint16_t BTE_height);
    void BTE_ROP_codeHelper(uint8_t code);
    void BTEWriteROP(int16_t dest_hor, int16_t dest_ver, uint16_t BTE_width, uint16_t BTE_height, uint16_t* img, uint32_t num);


    /*  DMA Function  */
    void enableDMAInt(void);
    void enableDMA(void);
    void startAddressDMA(uint32_t addr);
    void continuousDMA(uint16_t x, uint16_t y, uint32_t addr, int32_t transfNum);
    void blockDMA(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t addr, int16_t SPWR);
    void _waitBusy(uint8_t res); 

    /*  PWM Control Function */
    void enablePWM1(uint8_t clock);
    void disablePWM1(uint8_t clock);
    void setPWM1DutyCycle(uint8_t dc);
    void enablePWM2(uint8_t clock);
    void disablePWM2(uint8_t clock);
    void setPWM2DutyCycle(uint8_t dc);

    void writeReg(uint8_t register_addr, uint8_t val);
    uint8_t readReg(uint8_t register_addr);
};
#endif